...
// Preencher por URL (ex.: ?numCot=022/2026&codigo=ACX2077520&cliente=Nelson ...)
...
// Gera link com os campos de Dados Gerais
function gerarLink(){
...
async function enviarDadosGerais(file){
  ...
  const mailto = `mailto:${encodeURIComponent(EMAIL_DESTINO)}?subject=${encodeURIComponent(payload.subject)}&body=${encodeURIComponent(payload.message)}`;
...
function addFerrRow(){
  ...
  tr.innerHTML=`<td><input type="checkbox"/></td><td>Ferramenta</td><td><input type="number" step="0.01"/></td><td><input type="number" step="1" value="1"/></td><td><input type="number" step="1" value="1000"/></td><td><input type="number" step="1" value="60"/></td><td><input type="number" step="0.01"/></td>`;
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  fileInput.addEventListener('change',()=>{
    ...
    if(f.type.startsWith('image/')){
      const url = URL.createObjectURL(f); previewImg.src = url;
    }else{ previewImg.src=''; }
  });
...
function calcular(){
  ...
  kpisDiv.innerHTML = `
    ...
    <p><strong>Peso bruto por peça:</strong> ${massa_kg.toFixed(4)} kg &nbsp;|&nbsp; <strong>Peso líquido:</strong> ${peso_liquido.toFixed(4)} kg</p>
